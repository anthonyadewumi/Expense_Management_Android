 ...
 */
open class MileageExpensesPagingSource(
    private val apiHelper: ApiHelper,
    private val mileageRequest: MileageExpenseRequest
) : PagingSource<Int, MileageDetail>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MileageDetail> {
        val pageNumber = params.key ?: 1
        return try {
            mileageRequest.page = pageNumber
            val data = apiHelper.mileageExpensesList(mileageRequest)
            val nextKey = if(data.success) pageNumber.plus(1) else pageNumber
//            val prevKey = if (pageNumber == 1) null else pageNumber - 1

            if(data.success) {
                Page(
                    data = data.mileageList!!,
    ...
    override fun getRefreshKey(state: PagingState<Int, MileageDetail>): Int? {
