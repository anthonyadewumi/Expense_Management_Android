...
import com.bonhams.expensemanagement.data.services.responses.AcceptRequestResponse
 ...
 */
open class FinancePagingSource(
    private val apiHelper: ApiHelper,
    private val claimsRequest: ClaimsRequest
) : PagingSource<Int, ToBeAcceptedData>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ToBeAcceptedData> {
        val pageNumber = params.key ?: 1
        return try {
            claimsRequest.page = pageNumber
            val data = apiHelper.financeRequestList(claimsRequest)
            val nextKey = if(data.success) pageNumber.plus(1) else pageNumber
            val prevKey = if (pageNumber == 1) null else pageNumber - 1

            if(data.success) {
                Page(
                    data = data.tobeaccepteddata!!,
                ...
                Page(
                    data = emptyList<ToBeAcceptedData>(),
                    prevKey = null, // Only paging forward. prevKey,
    ...
    override fun getRefreshKey(state: PagingState<Int, ToBeAcceptedData>): Int? {
