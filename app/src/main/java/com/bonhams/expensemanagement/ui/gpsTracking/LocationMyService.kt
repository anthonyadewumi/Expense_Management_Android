...
class LocationMyService : JobService(), GoogleApiClient.ConnectionCallbacks,
    ...
    }//here we can show Alert to start location//Log.d(TAG, "getLastKnownLocation()");
     ...
     * starting job again
     */
    private fun startJobAgain() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Log.d(TAG, "Job Started")
            val componentName = ComponentName(
                applicationContext,
                LocationMyService::class.java
            )
            jobScheduler =
